    ...
    return (
        <div className="grid custom_grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 sm:gap-10 md:gap-12 ml-2 mr-2 px-8 sm:px-4 md:px-12 custom-height overflow-y-auto py-8 -mt-4">
            {items.map((item, index) => (
                <div key={index} onClick={() => setGestioneazaItem(item)} className="border border-white-700 rounded-lg w-68 h-32 shadow-lg shadow-gray-300 bg-white items-center hover:bg-gray-400 cursor-pointer group  transition-all duration-300 ease-in-out">
                    <div className="flex flex-col justify-center mt-1 opacity-0 group-hover:opacity-100 transition-opacity duration-100 space-x-2 relative">
                        ...
                        <button onClick={(e) => { e.stopPropagation(); setStergeItem(true); setItemid(item) }} className=" absolute mt-48 right-2 border border-white bg-white rounded-lg px-2 py-1 hover:bg-red-400">

...
const PopupStergeItemGroupDefinitiv = ({ setShowPopupStergeItem, item, items, fetchItems, idgrup }) => {
    ...
    const handleStergeItem = async () => {
        ...
        try {
            const response = await fetch('http://localhost:9000/api/grupuri/stergeItemGroupDefinitiv', {
                ...
                body: JSON.stringify({ id_item: item.id_item, id_grup: idgrup }),
                credentials: "include"
            });
...
export default PopupStergeItemGroupDefinitiv;